...
color 0F
echo ======================================================
echo       GBA BACKGROUND STUDIO - ASSISTENTE (PT)
echo ======================================================
echo.
...
set "INPUT_IMG="
echo [PASSO 1]
set /p "INPUT_IMG=Arraste a imagem (PNG/BMP) e prima Enter: "
...
echo     2. Rotacao/Escalamento (Affine)
set "MODE_CHOICE=1"
...
if "%TYPE_4BPP%"=="2" (
    set /p "TMAP_PATH=   Arraste o ficheiro .bin aqui: "
...
echo.
set "TRANS_CLR=0,0,0"
set /p "TRANS_CLR=[PASSO 7] Cor Transparente (R,G,B) [Padrao: 0,0,0]: "
set "C_ARGS=--transparent-color "%TRANS_CLR%""

set "KEEP_T=n"
set /p "KEEP_T=[PASSO 8] Manter cor transparente na paleta? (s/n) [n]: "
if /i "%KEEP_T%"=="s" set "C_ARGS=!C_ARGS! --keep-transparent"

echo.
set "EXT_T=0"
set /p "EXT_T=[PASSO 9] Tiles transparentes extras no inicio [Padrao: 0]: "
set "C_ARGS=!C_ARGS! --extra-transparent-tiles %EXT_T%"

set "TS_W="
set /p "TS_W=[PASSO 10] Forcar largura do Tileset (1-64) [Padrao: auto]: "
if defined TS_W set "C_ARGS=!C_ARGS! --tileset-width %TS_W%"

echo.
set "ORIG="
set /p "ORIG=[PASSO 11] Origem do recorte (ex: 1t,1t ou 8,8) [Padrao: 0,0]: "
if defined ORIG set "C_ARGS=!C_ARGS! --origin "%ORIG%""

set "C_END="
set /p "C_END=[PASSO 12] Final do recorte (ex: 30t,18t) [Opcional]: "
if defined C_END set "C_ARGS=!C_ARGS! --end "%C_END%""

echo.
set "OUT_S="
set /p "OUT_S=[PASSO 13] Tamanho de saida (screen, bg0, 32t,20t) [Padrao: Original]: "
if defined OUT_S set "C_ARGS=!C_ARGS! --output-size "%OUT_S%""

echo.
set "K_TEMP=n"
set /p "K_TEMP=[PASSO 14] Manter ficheiros temporarios? (s/n) [n]: "
if /i "%K_TEMP%"=="s" (set "EXT_OPTS=--keep-temp") else (set "EXT_OPTS=")

set "S_PREV=n"
set /p "S_PREV=[PASSO 15] Guardar antevisao em /output? (s/n) [n]: "
...
if %ERRORLEVEL% NEQ 0 ( color 0C & echo [ERRO] A conversao falhou. ) else ( color 0A & echo [SUCESSO] Ficheiros gerados. )
