...
class TilemapUtils:
    ...
    def _update_crop_mask(self):
        ...
        # Only show mask when shrinking in at least one dimension
        ...
        # Right strip (columns new_w..cur_w-1) for full height
        if new_w < cur_w:
            x = new_w * 8
            w = (cur_w - new_w) * 8
            h = min(new_h, cur_h) * 8
        ...
        # Bottom strip (rows new_h..cur_h-1) for full width
        if new_h < cur_h:
            y = new_h * 8
            w = cur_w * 8
            h = (cur_h - new_h) * 8
